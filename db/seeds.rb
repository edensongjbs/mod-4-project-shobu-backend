...
shobu_board.each_with_index do |board, board_index|
    4.times do |i1|
        4.times do |i2|
            coords = "#{i1}#{i2}"
            shobu_matrix[board_id][i1][i2] = Square.create(board: board, coordinates: coords)
        end
    end
end
...
4.times do |board_id|
    4.times do |row_id| 
        ...
        Move.create(done: true, read: true, aggressive: false, start_square: shobu_matrix[board_id][row_id][0], piece: the_piece)
    end
end
...
4.times do |board_id|
    4.times do |row_id| 
        ...
        Move.create(done: true, read: true, aggressive: false, start_square: shobu_matrix[board_id][row_id][3], piece: the_piece)
    end
end
